...
/**
 * La anotación @Configuration indica que esta clase se utilizará como una fuente de definiciones de beans para el contexto de la aplicación.
 * Los métodos anotados con @Bean dentro de una clase anotada con @Configuration se utilizan para definir beans de Spring.
 * En este caso, se utiliza la anotación @Configuration para indicar que esta clase se encargará de la configuración de seguridad de la aplicación.
 * <br>
 * La anotación @EnableWebSecurity se utiliza para habilitar la seguridad web en una aplicación Spring Boot.
 * Esta anotación importa automáticamente la clase WebSecurityConfiguration, que proporciona la configuración de seguridad predeterminada.
 * Cuando se utiliza @EnableWebSecurity, también se recomienda definir una clase que extienda WebSecurityConfigurerAdapter para personalizar la configuración de seguridad.
 * En este caso, @EnableWebSecurity se utiliza para habilitar la seguridad web en la aplicación y permitir la personalización de la configuración de seguridad a través de la clase SecurityConfig.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
    ...
    /**
     * Método que crea un proveedor de autenticación de DAO y lo configura con el userDetailsService y el passwordEncoder.
     *
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                /*
                 * Esta línea de código {@code .csrf().disable();} desactiva la protección contra ataques CSRF (Cross-Site Request Forgery) para todas las solicitudes en la aplicación web.
                 * CSRF es un tipo de ataque de seguridad en el que un atacante engaña al usuario para que ejecute una acción no deseada en un sitio web, utilizando para ello la sesión activa del usuario.
                 * La desactivación de la protección CSRF no es una práctica recomendada, pero en algunos casos puede ser necesario hacerlo,
                 * por ejemplo, para pruebas locales o en aplicaciones en las que la protección CSRF no es crítica.
                 * Si no desactivamos csrf, debemos tener en cuenta que funciona de manera automática con la libreria thymeleaf-extras-springsecurity6
                 * Thymeleaf reconocerá los formularios que creemos con el atributo "th:action" y de este modo les incluirá de manera automática el campo hidden _csfr
                ...
                .authorizeHttpRequests(authorize ->authorize
                        ...
                        .requestMatchers("/chat","/files/**","/upload","/userFiles/**","/database/**").authenticated()
